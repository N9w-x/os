...
pub struct Futex {
    waiters: Mutex<usize>,
    chain: Mutex<VecDeque<Arc<TaskControlBlock>>>,
}

impl Futex {
    pub fn new() -> Self {
        Self {
            waiters: unsafe { Mutex::new(0) },
            chain: unsafe { Mutex::new(VecDeque::new()) },
        }
    }
...
pub fn futex_wait(uaddr: usize, val: usize, timeout: usize) -> isize {
    ...
    if *uval == val as u32 {
        let mut futex_manager = FUTEX_MANAGER.lock();
        ...
        // };
        futex.waiters_inc();
        let mut futex_queue = futex.chain.lock();
...
pub fn futex_requeue(uaddr: usize, num_wake: usize, uaddr2: usize) -> usize {
    ...
    if let Some(futex) = futex_manager.get(&uaddr) {
        ...
        let mut futex2_queue = futex2.chain.lock();
